...
local XaaTzFolder = "XaaTz"
local ConfigurationFolder = XaaTzFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'XaaTz Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(XaaTzFolder) then
				if isfile and isfile(XaaTzFolder..'/settings'..ConfigurationExtension) then
					file = readfile(XaaTzFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"XaaTz Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"XaaTz Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('XaaTz had an issue accessing configuration saving capability.')
		end
	end
end
...
local XaaTz = useStudio and script.Parent:FindFirstChild('XaaTz') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local rayfieldDestroyed = false -- True when XaaTzLibrary:Destroy() is called

repeat
	if XaaTz:FindFirstChild('Build') and XaaTz.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('XaaTz | Build Mismatch')
		print('XaaTz may encounter issues as you are running an incompatible interface version ('.. ((XaaTz:FindFirstChild('Build') and XaaTz.Build.Value) or 'No Build') ..').\n\nThis version of XaaTz is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, XaaTz = XaaTz, useStudio and script.Parent:FindFirstChild('XaaTz') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(XaaTz)
	XaaTz.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	XaaTz.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == XaaTz.Name and Interface ~= XaaTz then
			Interface.Enabled = false
			Interface.Name = "XaaTz-Old"
		end
	end
elseif not useStudio then
...
if XaaTz.AbsoluteSize.X < minSize.X and XaaTz.AbsoluteSize.Y < minSize.Y then
...
local Main = XaaTz.Main
local MPrompt = XaaTz:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = XaaTz:FindFirstChild('Drag')
...
XaaTz.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/XaaTz/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = XaaTz.Notifications

local SelectedTheme = XaaTzLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = XaaTzLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	XaaTz.Main.BackgroundColor3 = SelectedTheme.Background
	XaaTz.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	XaaTz.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	XaaTz.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	XaaTz.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	XaaTz.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	XaaTz.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		XaaTz.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		XaaTz.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(XaaTz:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("XaaTz | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("XaaTz | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('XaaTz had an issue decoding the configuration file, please try delete the file and reopen XaaTz.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(XaaTzLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("XaaTz | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--XaaTzLibrary:Notify({Title = "XaaTz Flags", Content = "XaaTz was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(XaaTzLibrary.Flags) do
...
function XaaTzLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('XaaTz | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			XaaTzLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			XaaTzLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "rayfieldOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(XaaTzFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('XaaTz Settings', 0, true)

	if TabList['XaaTz Settings'] then
		TabList['XaaTz Settings'].LayoutOrder = 1000
	end

	if Elements['XaaTz Settings'] then
		Elements['XaaTz Settings'].LayoutOrder = 1000
	end
...
function XaaTzLibrary:CreateWindow(Settings)
	print('creating window')
	if XaaTz:FindFirstChild('Loading') then
		if getgenv and not getgenv().rayfieldCached then
			XaaTz.Enabled = true
			XaaTz.Loading.Visible = true

			task.wait(1.4)
			XaaTz.Loading.Visible = false
		end
	end
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				XaaTzLibrary:Notify({Title = 'Build Mismatch', Content = 'XaaTz may encounter issues as you are running an incompatible interface version ('.. ((XaaTz:FindFirstChild('Build') and XaaTz.Build.Value) or 'No Build') ..').\n\nThis version of XaaTz is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if isfolder and not isfolder(XaaTzFolder) then
		makefolder(XaaTzFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "XaaTz"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "XaaTz Interface Suite" then
		LoadingFrame.Version.Text = "XaaTz UI"
	end
	...
	if not Settings.DisableXaaTzPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				XaaTzLibrary:Notify({
					Title = "XaaTz Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(XaaTzFolder.."/Discord Invites") then
			makefolder(XaaTzFolder.."/Discord Invites")
		end

		if isfile and not isfile(XaaTzFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(XaaTzFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"XaaTz RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(XaaTzFolder.."/Key System") then
			makefolder(XaaTzFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("XaaTz | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with XaaTz specific development.')
				end
			end
		end
		...
		if isfile and isfile(XaaTzFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(XaaTzFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(XaaTzFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						XaaTzLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called XaaTzLibrary:Destroy()
				...
				if not Success then
					...
					print("XaaTz | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with XaaTz specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= XaaTzLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					XaaTzLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			XaaTz.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("XaaTz | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with XaaTz specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					XaaTzLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("XaaTz | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with XaaTz specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					XaaTzLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("XaaTz | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with XaaTz specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					XaaTzLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= XaaTzLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("XaaTz | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with XaaTz specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						XaaTzLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("XaaTz | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with XaaTz specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					XaaTzLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			XaaTzLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			XaaTzLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('XaaTz had an issue creating settings.') end
...
function XaaTzLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function XaaTzLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function XaaTzLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['XaaTz Settings'])
		end)
	end)
...
function XaaTzLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				XaaTzLibrary:Notify({Title = "XaaTz Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			XaaTzLibrary:Notify({Title = "XaaTz Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('XaaTz Configurations Error | '..tostring(result))
			XaaTzLibrary:Notify({Title = "XaaTz Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = XaaTzLibrary:CreateWindow({
	--	Name = "XaaTz Example Window",
	--	LoadingTitle = "XaaTz Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using XaaTz may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like XaaTz to get the key from
	...
	----XaaTzLibrary:Notify({Title = "XaaTz Interface", Content = "Welcome to XaaTz. These - are the brand new notification design for XaaTz, with custom sizing and XaaTz calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(XaaTzLibrary.Theme) do
...
task.delay(4, function()
	XaaTzLibrary.LoadConfiguration()
